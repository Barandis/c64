...
import {
  ...
  levelOptions, levelToggleHigh, levelToggleLow, levelToggleNone,
  levelUpdateNoTrace,
} from "./pin/level"
import {
  puInitial, puUnconnected, puInput, puOutput, puBidirectional, puAfter,
  pdInitial, pdUnconnected, pdInput, pdOutput, pdBidirectional, pdAfter,
  pnInitial, pnPullUp, pnPullDown,
} from "./pin/float"
import {
...
describe("Pin", () => {
  ...
  describe("mode", () => {
    ...
    it(
      "causes its trace to update after mode change from out to in",
      test(modeOutToIn)
    )
    it(
      "causes its trace to update after mode change from bidi to in",
      test(modeBidiToIn)
    )
    it(
      "does not change trace after mode change from unc to in",
      test(modeUncToIn)
    )
    it(
      "does not change trace after mode change from bidi to out",
      test(modeBidiToOut)
    )
    it(
      "causes its trace to update after mode change from unc to out",
      test(modeUncToOut)
    )
    it(
  ...
  describe("level", () => {
    ...
    it(
      "can be set for unconnected pins and does not affect its trace",
      test(levelUnconnected)
    )
    it("can only be set by its trace for input pins", test(levelInput))
    it("sets its trace's value for output pins", test(levelOutput))
    it(
      "sets and accepts values from its trace for bidirectional pins",
      test(levelBidirectional)
    )
    ...
    describe("pullUp", () => {
      ...
      it(
        "sets a pin's level if it is otherwise set to null",
        test(puUnconnected)
      )
      ...
      it(
        "will set a trace's level if a bidirectional pin goes null",
        test(puBidirectional)
      )
  ...
  describe("listeners", () => {
    it(
      "will not call a listener on an unconnected pin",
      test(listenerUnconnected)
    )
    ...
    it(
      "will call a listener on a bidirectional pin",
      test(listenerBidirectional)
    )
    it(
      "will not call a listener if the pin is changed directly",
      test(listenerDirect)
    )
    it("will not call a removed listener", test(listenerRemove))
    it(
      "will do nothing when removing an unadded listener",
      test(listenerNonexistent)
    )
    it(
