...
// ## Parallel Data Ports
//
// The pins PA0-PA7 and PB0-PB7 make up the two parallel data ports. The direction of each
// pin (input or output) is controlled individually by the corresponding bit in the Data
// Direction Register for each port. Setting a bit to 1 means that that pin is being used as
// an output; a 0 sets it to an input instead.
//
...
// ignored).
//
// Port B additionally can be used as outputs for the timers; PB6 can signal for Timer A and
// PB7 for Timer B. To do this, the PBON bit in the appropriate control register (CRA or
// CRB) must be set. The output mode is set with the OUTMODE bit in the same register.
// Setting this bit means that the output pin will toggle levels each time the timer reaches
// 0; clearing it means that a one-cycle-long high pulse will happen each time the timer
// reaches 0. This setting overrides the input/output settings; as long as one of the PBON
// bits is set, that Port B pin will be an output whatever is set in DDRB.
//
// Finally, the PC pin is associated with Port B. Each time Parallel Data Register B is read
// or written, the PC pin will go low for one cycle. This is useful as a signal that either
// the port's data has been accepted or that the port has data ready to be received.
//
// ## Interval Timers
//
// The two interval timers each track their countdown in pairs of registers - TALO and TAHI
// for Timer A and TBLO and TBHI for timer B - allowing them to count a maximum of 65,536
// cycles. The timers can count clock cycles (provided on pin PHI2) or transitions of the
// CNT pin to high. Given clock cycles with a 1 MHz clock frequency (typical of a Commodore
// 64), this makes for a maximum interval of about 0.066 seconds.
//
...
//
// The type of event to count is set in the control registers. CRA has a bit called INMODE;
// if this is 0, Timer A counts clock cycles, and if it's 1, Timer A counts CNT transitions.
// For Timer B, CRB has *two* bits (INMODE0 and INMODE1). A value of 00 or 01 on these bits
// is the same as for Timer A, but 10 causes Timer B to count Timer A underflows and 11
// means Timer B will count only the Timer A underflows that happen when the CNT pin is
// high.
//
// Once a timer reaches zero, a number of things can happen. First, an interrupt request may
// be signaled. This requires that the TA or TB bits in the register ICR be set. Secondly,
// an output can be signaled on PB6 (for Timer A) or PB7 (for Timer B). Details of this have
// already been discussed in the "Parallel Data Ports" section. Finally, the timer is reset
// and the decision is made to either stop there or to continue counting down. This is
// controlled by the RUNMODE bit in CRA or CRB. If this bit is 0, the timer resets and
// continues running; if it is 1, the timer still resets but does not re-run.
//
...
//
// The load will happen immediately if the LOAD bit of either CRA or CRB are set. Whatever
// is in the countdown registers will be overwritten with whatever is in the shadow
...
// (TODMIN), seconds (TODSEC), and tenths of seconds (TOD10TH). Once set, the TOD clock
// keeps accurate time as long as the signals on the clock pins are accurate.
//
// Reading these registers reveals the current time, but it has to be done in a particular
// manner. Reading TODHR causes all of the registers to stop updating until TOD10TH is read.
// This is to ensure that all four registers can be read at the same "time". Otherwise, it
// is possible (for example) to start reading the time at 1:59:59.9 and have time second
// tick over before the read is finished, making the first read happen before the hour
// changes and the others happen after. If this happened, the time would appear to be
// 1:00:00.0 when it really should be 2:00:00.0. The clock itself is not stopped during this
// pause; the clock merely doesn't update the registers until TOD10THS is next read.
//
// Writing the TOD registers works the same way, with a couple of additional caveats. First,
// the clock itself stops after TODHR is written until TOD10THS is also written. Second, it
// is possible that writing to these registers doesn't change the time at all; if the ALARM
// bit of CRB is set, then writing these registers changes the *alarm*, which is kept in
// shadow registers that can't be read.
//
...
//
// Because of this BCD formatting, we know that the top four bits of TOD10TH will always be
// 0 (since the highest number it will hold is 09). Similarly, the top bit of TODSEC and
// TODMIN (max values 59) will always be 0. The top 3 bits of TODHR (max value 12; there is
// no support for 24-hour time) would be always 0 except that bit 7 (PM) is used to indicate
// AM (0) or PM (1).
//
// The clock operates on external clock pulses fed to the TOD pin. By default, these pulses
// are expected to arrive at 60 Hz (in other words, TOD10THS will be incremented after every
// sixth pulse). If the TODIN bit of CRA is set, it will instead assume the clock frequency
// is 50 Hz (and TOD10THS will instead be updated every fifth clock pulse). Obviously, the
// accuracy of these pulses is important to having the correct time.
//
...
//
// Whether the serial port is being used to send or receive data is controlled by SPMODE bit
// of register CRA (0 for receive and 1 for send). If receiving, the chip pushes a bit from
// the SP pin to an internal shift register every time the CNT pin goes high (the sending
// device is responsible for signaling CNT). Once 8 bits have been received, they are
// deposited in the SDR register and an interrupt request is generated.
//
...
// the CNT pin.) Once the entire byte is sent, an interrupt request is generated; if the CPU
// had already put a new byte into SDR the transmission will continue, otherwise it will be
// paused until the next byte is ready.
//
// ## Control Registers
//
...
// requests an interrupt when it goes low. A potential use of this is tying it to the PC pin
// of another 6526 to request an interrupt when parallel port data is sent or received
...
//
// The masks instead affect the IR bit. If the event happened *and* the ICR had been written
// with a value that turned the bit for that event on, then the IR bit will also be set and
...
//
// The START bit indicates whether Timer A is running. If it's at 0, setting it to 1 is what
// starts the timer. If Timer Ais in one-shot mode (see RUNMODE below), then when it
...
// output with PBON. If this is 0, then PB6 will go high for one clock cycle when Timer A
// underflows. If it's 1, then PB6 will toggle between high and low each time Timer A
// underflows.
//
...
// writing a 0 to it has no effect). When CRA is written with a 1 in this bit position, the
// contents of the shadow register for each timer register will be immediately loaded into
// its respective timer register. In this way, the timers can be reset without having to
// wait for them to underflow (which is the only other way that the contents of the
// registers are reset).
//
// The INMODE bit determines what Timer A actually counts. If this is 0, Timer A counts
// pulses on the PHI2 pin (the system clock). If this is 1, Timer A counts pulses on the CNT
// pin (which has no fixed function; any external circuit can provide pulses for CNT).
//
...
// not, of course, change the actual clock frequency on the TOD pin; this clock is supplied
// externally. It simply changes how the TOD clock interprets those pulses. (In other words,
// setting this to 50Hz while supplying a 60Hz external clock will make for a TOD clock that
// runs very fast.)
//
...
// | Bit        | 7     | 6       | 5       | 4    | 3       | 2       | 1    | 0     |
// | ---------- | ----- | ------- | ------- | ---- | ------- | ------- | ---- | ----- |
// | Function   | ALARM | INMODE1 | INMODE0 | LOAD | RUNMODE | OUTMODE | PBON | START |
//
// The lower 5 bits of this register are identical to the lower 5 bits of CRA except that
// they affect Timer B instead of timer A, and that PBON sets the behavior of pin PB7 rather
// than PB6.
//
// In this register, INMODE takes two bits (marked INMODE0 for the lower bit and INMODE1 for
// the upper). This is because in addition to the two events that Timer A can count, Timer B
// can count two more.
//
// * 00: System clock (pin PHI2) pulses
// * 01: CNT pin pulses
...
// PA0-PA7 and PB0-PB7 are internally pulled up. If nothing is connected to them or if no
// data is coming into an input pin, that pin will read as a 1 when the PRA or PRB registers
// are read.
//
// Those registers are actually physically read and written through the address pins A0-A3
// and the data pins D0-D7. The offset listed in the very first table in this description is
// what must be on the A pins, while the D pins either have the data to write to that
// register or are set to the data to be read. Whether an operation is to be a read or write
// is actually set by the R_W pin.
//
...
// | --- | ----- | ----------------------------------------------------------------------- |
// | 2   | PA0   | Parallel data port A input/output pins. Whether these are sending or    |
// | 3   | PA1   | receiving depends on the values in the DDRA register. If one of the     |
// | 4   | PA2   | pins is an output, the value in the PRA register for that bit will      |
// | 5   | PA3   | appear on the pin; if a pin is an input, whatever value is put onto it  |
// | 6   | PA4   | will be reflected in that bit of the PRA register.                      |
// | 7   | PA5   |                                                                         |
// | 8   | PA6   |                                                                         |
// | 9   | PA7   |                                                                         |
// | --- | ----- | ----------------------------------------------------------------------- |
// | 10  | PB0   | Parallel data port B input/output pins. Whether these are sending or    |
// | 11  | PB1   | receiving depends on the values in the DDRB register. If one of the     |
// | 12  | PB2   | pins is an output, the value in the PRB register for that bit will      |
// | 13  | PB3   | appear on the pin; if a pin is an input, whatever value is put onto it  |
// | 14  | PB4   | will be reflected in that bit of the PRB register.                      |
// | 15  | PB5   |                                                                         |
// | 16  | PB6   | PB6 and PB7 can instead be used as outputs for Timer A and Timer B      |
// | 17  | PB7   | respectively depending on settings in CRA and CRB.                      |
// | --- | ----- | ----------------------------------------------------------------------- |
// | 18  | PC    | Active-low handshaking output pin. This goes low for one cycle each     |
// |     |       | time PRB is read or written.                                            |
// | --- | ----- | ----------------------------------------------------------------------- |
// | 19  | TOD   | Time-of-day clock input. This is expected to be a 50Hz or 60Hz clock    |
// |     |       | (depending on settings in the CRA register).                            |
// | --- | ----- | ----------------------------------------------------------------------- |
// | 20  | Vcc   | +5V power supply. Not emulated.                                         |
// | --- | ----- | ----------------------------------------------------------------------- |
// | 21  | IRQ   | Active-low interrupt request output. This is set low any time an        |
// |     |       | interrupt condition is met. What these conditions are is determined by  |
// |     |       | settings in the ICR register.                                           |
// | --- | ----- | ----------------------------------------------------------------------- |
// | 22  | R_W   | Read (high) or write (low) control for the registers.                   |
// | --- | ----- | ----------------------------------------------------------------------- |
// | 23  | CS    | Active-low chip select. Must be low for register reading and writing.   |
// | --- | ----- | ----------------------------------------------------------------------- |
// | 24  | FLAG  | Active-low handshaking input pin. When this pin is set low, it          |
// |     |       | potentially generates an interrupt (depending on ICR settings).         |
// | --- | ----- | ----------------------------------------------------------------------- |
// | 25  | PHI2  | Clock input. Should be 1MHz; other values will shift timer frequencies. |
// | --- | ----- | ----------------------------------------------------------------------- |
// | 26  | D7    | Data pins. Data to be written to registers should be given to these     |
// | 27  | D6    | pins, and data read from registers will appear on them.                 |
// | 28  | D5    |                                                                         |
// | 29  | D4    |                                                                         |
// | 30  | D3    |                                                                         |
// | 31  | D2    |                                                                         |
...
// | 33  | D0    |                                                                         |
// | --- | ----- | ----------------------------------------------------------------------- |
// | 34  | RES   | Active-low reset pin. When this goes low, the chip resets.              |
// | --- | ----- | ----------------------------------------------------------------------- |
// | 35  | A3    | Address pins, controlling which register is read from/written to. Four  |
// | 36  | A2    | pins can address the 16 registers present on the chip.                  |
// | 38  | A0    |                                                                         |
// | 38  | A0    |                                                                         |
// | --- | ----- | ----------------------------------------------------------------------- |
// | 39  | SP    | Serial port input/output. Data from the SDR register appears here one   |
// |     |       | bit at a time in output mode, and data that is set here from outside    |
// |     |       | will fill the SDR register one bit at a time in input mode.             |
// | --- | ----- | ----------------------------------------------------------------------- |
// | 40  | CNT   | Count pin. The CRA and CRB registers can be configured to use pulses on |
// |     |       | this pin rather than clock pulses as the clock for the timers and for   |
// |     |       | the serial port.                                                        |
//
