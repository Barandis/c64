...
// data port, two microsecond-accuracy timers, a BCD-based time-of-day
// clock, and 16 registers to interface with the microprocessor.
//
// The Commodore 64 has two 6526 CIAs. The first is used almost
// exclusively to read the keyboard, joysticks, and joystick/paddle
// button. (It also provides one serial port to the User Port and fires
// an interrupt when the cassette tape is read.) Its interrupts are
// provided to the CPU over the regular IRQ line; these interrupts
// typically happen 60 times a second for keyboard scanning in addition
// to whatever may come from the serial port or the cassette tape.
//
// The second CIA mostly controls the Serial Port and the parallel
// (RS-232) port available on the User Port, though it's also used to
// provide bank switching to the VIC. Its interrupts are provided to the
// CPU over the NMI line, ensuring that they're processed without delay.

export function Ic6526() {
  const chip = Chip(
    // Register select pins. The 6526 has 16 addressable 8-bit
