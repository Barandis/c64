 ...
 *
 * Finally, the `_PC` pin is associated with Port B. Each time Parallel Data Register B is
 * read or written, the `_PC` pin will go low for one cycle. This is useful as a signal that
 ...
 * of 65,536 cycles. The timers can count clock cycles (provided on pin `φ2`) or transitions
 * of the `CNT` pin to high. Given clock cycles with a 1 MHz clock frequency (typical of a
 * Commodore 64), this makes for a maximum interval of about 0.066 seconds.
 *
 ...
 *
 * This marks the first appearance of the `_FLAG` pin; this is simply a pin that potentially
 * requests an interrupt when it goes low. A potential use of this is tying it to the `_PC`
 ...
 * bit *will* fire an interrupt (i.e., the `_IRQ` pin will go low) when that event happens.
 ...
 * be set and the `_IRQ` pin will go low.
 *
 ...
 *
 * In this scenario, when Timer A underflows for the first time, the `_IRQ` pin will not be
 ...
 * requested (the `IR` bit is 0 and the `_IRQ` pin is not low). This is because the Timer A
 ...
 * was set to count 2 Timer A underflows). This time, the `_IRQ` pin *will* go low, and
 ...
 * read or write is actually set by the `R__W` pin.
 *
 * `_RES` resets the chip when it's held low. Resetting makes all registers go to 0 (which,
 ...
 * its `_FLAG` pin) that fires when the cassette is ready to be read. CIA2 controls the user
 ...
 * interrupts are tied to the CPU's `_NMI` pin rather than the regular `_IRQ` pin, so they
 * will be handled immediately.
 *
 * @typedef Ic6526
 * @property {Pin} _CS [23] The active-low chip select pin. When this goes low, a read or
 *     write operation using the chips' registers is performed.
 * @property {Pin} _RES [34] The active-low reset pin. When this pin goes low, all of the
 *     chip's registers are reset.
 * @property {Pin} R__W [22] The read-write pin (normally written `R/_W`, but the slash is
 *     problematic in property names). If this is high, then a register access operation is
 *     a read. If it's low, the access operation is a write.
 * @property {Pin} _IRQ [21] The interrupt request output. The chip pulls this low when an
 *     interrupt event happens that is not masked off.
 * @property {Pin} _PC [18] This pin goes low for a clock cycle when parallel port B is read
 *     or written.
 * @property {Pin} _FLAG [24] An external interrupt request pin. If this goes low (and the
 *     appropriate mask is set), the chip will trigger an interrupt request.
 * @property {Pin} CNT [40] The count pin. This pin can receive pulses that can decrement
 *     the interval timers or indicate that a new bit has been sent to the serial port, or
 *     it can act as an output pin that goes high each time a bit is sent out the serial
 *     port.
 * @property {Pin} φ2 [25] The system clock. Clock pulses (1MHz on the C64) are supplied to
 *     this pin from the clock source, normally the microprocessor.
 * @property {Pin} TOD [19] The time-of-day clock. Clock pulses supplied here must be either
 *     60Hz or 50Hz. These pulses control the timing of the time-of-day clock.
 * @property {Pin} SP [39] The serial port. This pin sends or receives one bit at a time,
 *     clocked either by Timer A (if sending) or the `CNT` pin (if receiving).
 ...
 * @property {Pin} Vcc [28] The positive power supply. This pin is not emulated.
