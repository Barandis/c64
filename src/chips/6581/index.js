...
export function new6581() {
  ...
  // The 32 addressable registers on the 6581. Only 29 of these are actually used; reading the
  // others will always return 0xff and writing them will have no effect.
  const registers = new Uint8Array(32)

  // -----------------------------------------------------------------------------------------------
  // Reset

  // The spec says that _RES must be low for at least 10 cycles before a reset will occur. This
  // variable is set to 0 when _RES first goes low, and each φ2 cycle increments it. When it gets to
  // 10, `reset()` will be called.
  let resetClock = 0

  // Flag to know whether the chip has reset since the last time _RES went low. This is used to
  // ensure that the chip only resets once, rather than once every 10 cycles as long as _RES is held
  // low.
  let hasReset = false

  // This is the result of a reset according to the specs of the device. This is pretty simple since
  // the only outputs are the data lines and the audio out; all registers are set to zero, audio
  // output is silenced, and data lines are set back to their normal unconnected state.
  //
  // Since the three unused registers always return 0xff, we just set that here and keep it from
  // changing.
  //
  // TODO: For now, we're simply setting the registers directly rather than using the
  // `writeRegister` function to do it because there are no side effects in writing them; when sound
  // is implemented this may or may not change.
  function reset() {
    for (let i = 0; i < 32; i++) {
  ...
  // Keeps track of how many cycles have passed since the last write to a write-only register. If it
  // has exceeded max, then lastWriteValue becomes 0.
  chip.φ2.addListener(pin => {
    if (pin.high) {
      lastWriteTime++
  ...
  //
  // The three unused registers always return 0xff. The write-only registers return the value of the
  // last write made to *any* SID register. However, in the real chip this last-write value 'fades'
  // over time until, after 2000-4000 clock cycles, it is zero. The model for this fading is unknown
  // and is not properly emulated here; this emulation simply returns the last written value as long
  // as the last write has happened in the last 2000 cycles; otherwise it returns 0.
  ...
  // Writes a value to a register. This does not affect the read-only and unused registers, and the
  // high byte of the pulse-width registers only sees its low 4 bits written (the high 4 bits
  // remain, as always, 0).
  function writeRegister(index, value) {
    if (index === PWHI1 || index === PWHI2 || index === PWHI3) {
      // Strip the upper four bits so that the register always has 0's there
