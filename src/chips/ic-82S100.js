...
// An emulation of the Programmable Logic Array (PLA) used to control
// banking and memory access in the Commodore 64.
//
// The 82S100 was a programmable logic chip made by Signetics and is
...
// (P-terms) by logically AND- or NANDing selections of the 16 inputs.
...
// ultimately sent to the output pins. The programming for these P- and
// S-terms could be done in the field in a similar manner to that of a
// PROM (programmable read-only memory), and Commodore would program
// them as a part of production of the C64.
//
...
//
// The input pins of the 82S100 were generically named I0, I1, I2, ...
// I15, and the output pins were similarly named F0, F1, ... F7. That
// has been maintained here, though constants are provided to be able to
// use the more C64-centric names that reflect the pins' actual
// functions in that computer.
//
// This chip was tasked with providing the chip enable signals for the
// C64's RAM, BASIC ROM, KERNAL ROM, character ROM, I/O memory, and
// cartridge ROM. The 6510 processor, having a 16-bit address bus, could
// access 64k of memory, but all of this RAM and ROM together added up
// to closer to 84k. Therefore the PLA used a combination of inputs,
// including some of the address bus, the specialized 6510 I/O port bus,
// and signals from the VIC and cartridge ROMs to determine which memory
// was trying to be accessed at any given time, and then provide chip
// enable signals for that memory (turning it on and turning other
// memory that could be at the same address off). It would do this for
// every memory access, which would happen twice per cycle (half the
// cycle was used by the CPU, half by the VIC). Thus bank switching
// actually happened at a frequency of 2MHz, twice the CPU clock
// frequency, and the chip generated a fair bit of heat.
//
// Another issue with this chip in the C64 was the _CASRAM signal. This
// line is connected directly to the _CAS inputs on the DRAM chips,
// while the _CAS signal from the VIC (which also is fed to the PLA's
// _CAS input) would control the multiplexer that controlled whether the
// high or low address lines were visible to the DRAM. Since the
// multiplexer took a few nanoseconds to switch its outputs, the PLA had
// to provide a delay on the output of the _CASRAM to make sure that the
// multiplexers were ready to feed the correct address to the DRAM. This
// was easily done in the 82S100 since its 90ns response time gave
// plenty of time for the multiplexers to change state, but replacement
// PLA's have often had to deal with delaying the _CASRAM line because
// the replacement chips are too fast.
//
// Fortunately, that's not an issue that an emulator program needs to
// worry about.
//
// All of the information used in this file for logic equations came
// from
// http://skoe.de/docs/c64-dissected/pla/c64_pla_dissected_a4ds.pdf. If
// you're into that sort of thing, it's also an excellent read for all
// aspects of what's actually a pretty interesting chip (including why
// Commodore engineers eventually got banned from putting their initials
// onto chips they designed).
//
...
// accessed regularly with `chip.I0` or `chip.pins.I0`. With these
// constants, if so desired, it can also be accessed as
// `chip.pins[_CAS]`.
...
export function Ic82S100() {
  ...
  function oneListener() {
    ...
    //
    // The purpose of the logic in the PLA is to take the current state
    // of the system during a memory access and produce a single output
    // enabling the one device (memory chip, processor chip with
    // register, cartridge, etc.) that will handle that memory access.
    // (If the selected output is _IO, that indicates that the I/O block
    // of memory is being accessed, but the PLA cannot determine *which*
    // device is being accessed from this information alone. When that
    // happens, a separate demultiplexer determines which device it is
    // from A8...A11.)
    //
    // These are the inputs to the PLA.
    //
    // I0: _CAS. This is the control line from the VIC that enables
    //     access to RAM. Instead of going directly to RAM, this line
    //     comes to the PLA and if RAM access is selected, the _CASRAM
    //     output is sent to the RAM chips.
    //
    // I1: _LORAM. This line controls the memory block from
    //     $A000...$BFFF. When high (normal), the BASIC ROM is available
    //     in this memory block. When low, it's switched out for RAM.
    //
    // I2: _HIRAM. This line controls the memory area from
    //     $E000...$FFFF. When high (normal), the KERNAL ROM is
    //     available in this memory block. When low, it's switched out
    //     for RAM.
    //
    // I3: _CHAREN. This line partially controls the memory block from
    //     $D000...$DFFF. When high (normal), the I/O devices appear in
    //     this memory block. When low, the character ROM appears here
    //     instead. This line can be overridden by other signals,
    //     including those allowing this memory block to contain RAM
    //     instead.
    //
    // I4, I14-I15: The VIC address lines _VA14, VA13, and VA12. When
    //     the VIC is active, these are used to determine which block of
    //     memory the VIC is trying to access. _VA14 is active low
    //     because that's the way it's generated by CIA2; it's inverted
    //     back into active high before being used to access memory but
    //     not for use with the PLA.
    //
    // I5-I8: Address bus lines A15...A12. Used to determine which block
    //     of memory the CPU wishes to access while the CPU is active.
    //
    // I9: BA (Bus Available). The VIC sets this line high to indicate
    //     normal bus operation, switching between VIC and CPU each
    //     cycle. The VIC sets it low when it intends to take exclusive
    //     access of the address bus.
    //
    // I10: _AEC (Address Enable Control). This signal indicates which o
    //     the CPU (low) or VIC (high) is in control of the data bus.
    //
    // I11: R__W (written R/_W in places where slashes are legal). A
    //     high signal means that memory is being read, while a low
    //     means memory is being written. This makes a difference to
    //     what memory is being accessed. For example, ROM cannot be
    //     written; if an attempt is made to write to a ROM address
    //     while ROM is banked in, the write will happen to RAM instead.
    //
    // I12-I13: expansion port signals _GAME and _EXROM. These are used
    //     by cartridges to indicate the presence of addressable memory
    //     within them. There are two for-sure states for these two
    //     signals: if both are high (the normal state), there is no
    //     cartridge; and if _EXROM is high and _GAME is low, there is
    //     an Ultimax cartridge. Further mapping to cartridge ROM
    //     depends not only on these two signals but also on _LORAM,
    //     _HIRAM, and _CHAREN; there is a nice table at
    //     https://www.c64-wiki.com/wiki/Bank_Switching#Mode_Table.
    //
    // These inputs are combined and AND and NOT operations into P-terms
    // to determine which output will be selected. Each P-term below has
    // a comment with three lines. The first line describes the state of
    // the three 6510 I/O port lines that are used for bank switching
    // (_LORAM, _HIRAM, and _CHAREN). The second line is the memory
    // address that needs to be accessed to select that P-term (this is
    // from either the regular address bus when the CPU is active or the
    // VIC address bus when the VIC is active). The final line gives
    // information about whether the CPU or the VIC is active, whether
    // the memory access is a read or a write, and what type (if any) of
    // cartridge must be plugged into the expansion port (the cartridge
    // informaion takes into account the values of _LORAM, _HIRAM, and
    // _CHAREN already).
    //
    ...
    //
    // F0: _CASRAM. This is the signal that ultimately enables RAM. The
    //     production of this signal is very different from the others.
    //     For all other output, if one of their terms is selected, that
    //     output will be selected. For _CASRAM, its terms are combined
    //     with the terms from all other outputs (except for GR/_W), and
    //     if any of the terms are selected, then _CASRAM will be
    //     *de*selected.
    //
    ...
    //
    // F4: GR__W (written GR/_W where slashes are legal). Indicates that
    //     the static color RAM is being written to. Note that this is
    //     the only output that is not actually a chip enable signal,
    //     and also that I/O decoding must still enable color RAM for
    //     either a read *or* a write to be possible.
    //
    // F5: _IO. Indicates that one of the following are going to be
    //     enabled: CIA1 registers, CIA2 registers, VIC registers, SID
    //     registers, color RAM, expansion port I/O from address
    //     $DE00...$DEFF, or expansion port I/O from address
    //     $DF00...$DFFF. Which of these is actually enabled is done by
    //     decoding A8...A11, which other hardware does.
    //
