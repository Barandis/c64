...
export function control(chip, registers, latches) {
  ...
  //
  // Controls sending interrupts from the five sources available in the
  // 6526: Timer A underflow, Timer B underflow, TOD alarm, serial port
  // full/empty, and _FLAG pin clearing. One bit in the register is
  // assigned to each source; if an interrupt is generated, the bit
  // corresponding to the source(s) will be set.
  //
  // Writing to the register is slightly more complicated. The write
  // affects an internal latch rather than the register itself and sets
  // or clears one or more bits; whether it is set or clear depends on
  // bit 7 of the written value (1 = set, 0 = clear). Any set bit in the
  // rest of the value determines *which* ICR bit is set or cleared
  // (except for bits 5 and 6, which are ignored because there are only
  // five settable bits). These bits then act as a mask; if the bit in
  // this latch corresponding to the interrupt event is set, then the
  // interrupt will fire. If that bit is not set, the interrupt will be
  // ignored.
  //
  // When the register is read, it is cleared, so it is the
  // responsibility of the reader to store this information elsewhere if
  ...
  //
  // These two registers are primarily for controlling the two timers,
  // though they also help control the serial port and the TOD alarm.
