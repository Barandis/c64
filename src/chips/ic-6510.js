...
export class Ic6510 extends Chip {
  constructor() {
    ...
    // Address bus pins A0...A15.
      ...
      // signal on the input is merely forwarded to the output as a
      // clock reference for other chips on the board.
      new Pin(1, 'φ0', INPUT),
      ...
      // Address enable control. When this is low, the CPU tri-states
      // its busses to allow other chips to control them.
      new Pin(5, 'AEC', INPUT),

      // Interrupts. _IRQ is a normal interrupt that can be masked
      // (disabled) by setting the I flag in the P register to 1. _NMI
      // is a non-maskable interrupt that will fire even if the I flag
      // is set.
      ...
      // to three in a row). It will then go inactive until this pin
      // goes high again.
      new Pin(2, 'RDY', INPUT),
