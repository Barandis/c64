 ...
 * Commodore 128, and COCO2 switched to the 4464.
 *
 ...
 * row address is put onto the address pins and  the active-low row address strobe pin
 * `_RAS` is set low, then the column address is put onto the address pins and the
 * active-low column address strobe pin `_CAS` is set low.
 *
 * The chip has three basic modes of operation, controlled by the active-low write-enable
 * (`_WE`) pin with some help from `_CAS`. If `_WE` is high, then the chip is in read mode
 * after the address is set. If `_WE` is low, the mode depends on whether `_WE` went low
 * before the address was set by putting `_CAS` low; if `_CAS` went low first, (meaning the
 * chip was initially in read mode), setting `_WE` low will start read-modify-write mode,
 * where the value at that address is still available on the data-out pin (`Q`) even as the
 * new value is set from the data-in pin (`D`). If `_WE` goes low before `_CAS`, then read
 ...
 *
 * The Commodore 64 does not use read-modify-write mode. The `_WE` pin is always set to its
 * proper level before the `_CAS` pin goes low.
 *
 * While `_WE` and `_CAS` control what is read from and/or written to the chip's memory,
 * `_RAS` is not needed for anything other than setting the row address. Hence `_RAS` can
 * remain low through multiple memory accesses, as long as its address is valid for all of
 * them, allowing reads and writes to happen within a single 256-address page of memory
 ...
 * The combination of `_RAS` and `_CAS` can be regarded as such a pin, and it is used that
 * way in the Commodore
 * 64.
 *
 ...
 *
 * This chip is produced by calling the `{@link module:chips.Ic4164|Ic4164}` function.
 *
 * @typedef Ic4164
 * @property {Pin} _RAS [4] The active-low row address strobe pin. When this pin transitions
 *     to low, the value on the 8 address pins is latched and remembered as 8 bits of the
 *     final address.
 * @property {Pin} _CAS [15] The active-low column address strobe pin. When this pin
 *     transitions to low, the value on the 8 address pins is latched and remembered as
 *     other 8 bits of the final address. If the `_WE` is high at this time, a bit is read
 *     from the final address and put onto the `Q` pin; if `_WE is low, the value on the `D`
 *     pin is stored at the final address.
 * @property {Pin} _WE [3] The active-low write enable pin. If this pin is low when `_CAS`
 *     goes low, the bit on the `D` pin will be written to memory. If it's high, a bit read
 *     from memory will be put on the `Q` pin.
 ...
 * @property {Pin} NC [1] No contact. This pin is not connected to anything internally; the
 *     chip only needs 15 pins and this one is included to make the physical package a
 *     16-pin DIP.
 * @property {Pin} Vcc [8] The positive power supply. This pin is not emulated.
...
export default class Ic4164 extends Chip {
  ...
  constructor() {
    super(
      ...
      // The write-enable pin. If this is high, the chip is in read mode; if it and _CAS are
  ...
  // Invoked when the _RAS pin changes state. When it goes low, the current states of the
  // A0-A7 pins are latched. The address is released when the _RAS pin goes high.
  //
  // Since this is the only thing that _RAS is used for, it can be left low for multiple
  ...
  //
  // When _CAS goes low, the current states of the A0-A7 pins are latched in a smiliar way
  // to when _RAS goes low. What else happens depends on whether the _WE pin is low. If it
  // is, the chip goes into write mode and the value on the D pin is saved to a memory
  // location referred to by the latched row and column values. If _WE is not low, read mode
  // is entered, and the value in that memory location is put onto the Q pin. (Setting the
  // _WE pin low after _CAS goes low sets read-modify-write mode; the read that _CAS
  // initiated is still valid.)
  //
  // When _CAS goes high, the Q pin is disconnected and the latched column and data (if
  // there is one) values are cleared.
  #casListener() {
    return pin => {
      if (pin.low) {
  ...
  //
  // When _WE is high, read mode is enabled (though the actual read will not be available
  // until both _RAS and _CAS are set low, indicating that the address of the read is
  // valid). The D pin is disconnected and the internal latched input data value is cleared.
  //
  // When _WE goes low, the write mode that is enabled depends on whether _CAS is already
  // low. If it is, the chip must have been in read mode and now moves into
  // read-modify-write mode. The data value on the Q pin remains valid, and the valus on the
  // D pin is latched and stored at the appropriate memory location.
  //
  // If _CAS is still high when _WE goes low, the Q pin is disconnected. Nothing further
  // happens until _CAS goes low; at that point, the chip goes into write mode (data is
