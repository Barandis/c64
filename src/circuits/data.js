...
export function DataCircuit(
  ...
  // U9...U12 and U21...U24 have two unidirectional data pins each, one in and one out. For
  // this purpose they're simply tied together.
  ...
  // Once again, the VIC complicates things, though far less so than with the address bus.
  // The VIC has 12 data pins, and the upper four are used exclusively to read from color
  // RAM. The CPU can also see color RAM and in fact is the only device that can write to
  // it.
  //
  // The CPU reads and writes color RAM through D0...D3, as expected. However, when the VIC
  // reads color RAM, it does it through D8...D11 and may in fact do that at the same time
  // as it reads other data via D0...D7. To keep color data from polluting D0...D7 at that
  // time, the connection that the CPU needs to send data to color RAM on D0...D3 needs to
  // be severed. That's the purpose of switch U16.
  D0.addPins(U16.A4)
  D1.addPins(U16.A3)
  D2.addPins(U16.A2)
  D3.addPins(U16.A1)
